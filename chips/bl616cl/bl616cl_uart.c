/****************************************************************************
 * apps/vendor/bouffalolab/chips/bl616cl/bl616cl_uart.c
 *
...
{
  return (bl616cl_serialin(priv, UART_FIFO_CONFIG_1_OFFSET) &
          UART_TX_FIFO_CNT_MASK) >>
         UART_TX_FIFO_CNT_SHIFT;
}

static unsigned int bl616cl_rxfifo_count(struct bl616cl_uart_s *priv)
{
  return (bl616cl_serialin(priv, UART_FIFO_CONFIG_1_OFFSET) &
          UART_RX_FIFO_CNT_MASK) >>
         UART_RX_FIFO_CNT_SHIFT;
}
...
static void bl616cl_lowsetup(void)
{
  ...
  tx_cfg = ((8 + 4) << UART_CR_UTX_BIT_CNT_D_SHIFT) |
           (1 << UART_CR_UTX_BIT_CNT_P_SHIFT) | UART_CR_UTX_FRM_EN;
