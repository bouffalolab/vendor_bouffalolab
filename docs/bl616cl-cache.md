...
和 coherent ABI。这个缺口会影响 XIP 代码更新、RAM 中动态代码、DMA buffer 的
...
| coherent D-clean -> I-invalidate | 纳入 | RAM 可执行代码和共享 buffer 需要固定顺序 |
...
- `chips/bl616cl/Kconfig` 提供 `BL616CL_CACHE` 以及 test-only `BL616CL_CACHE_TEST`。
...
| `BL616CL_CACHE_TEST` | `n` | test trace、硬件 bypass、分块限制和 MHCR 读取 | 产品无 test hook 和诊断符号 |
| `BL_OS_FEATURE_TESTS_CACHE` | `n` | `cache_test` 单一测试 main | 无测试应用和相关字符串 |
...
8. test-only hook 只能观测和 bypass 硬件调用，不能改变正式输入校验和清理语义。
...
公共 ABI 而没有 test hook/app；测试态才同时包含 test hook、trampoline 和应用。
defconfig 由 menuconfig/savedefconfig 生成，不手工维护 `.config`。

## 固件运行与实测数据

测试固件启动到 `NuttShell (NSH) NuttX-3.6.1`，`help` 可见 `cache_test`。最终功能
验证执行 CACHE-001..008：getter 发布 I=32 KiB、D=16 KiB；all/range 路由、地址域、
溢出分块、partial invalidate、RAM 代码 coherent、runtime toggle、alias 可见性和
non-cacheable stack 操作全部 PASS，汇总为 `pass=8 fail=0 -> PASS`。容量
working-set/HPM 探针不属于最终验收。

同一固件的外设回归也通过：GPIO edge 三周期、TIMER-001 最大误差 0.647%、TIMER-002
